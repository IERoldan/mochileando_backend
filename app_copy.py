...
class myCards1(db.Model):  # Producto hereda de db.Model
    """
...
class myCards1Schema(ma.Schema):
    """
...
myCard_schema = myCards1Schema()  # Objeto para serializar/deserializar un producto
myCards1_schema = myCards1Schema(many=True)  # Objeto para serializar/deserializar múltiples productos
...
'''
@app.route("/myCards1", methods=["GET"])
def get_myCards():
    """
    ...
    """
    all_cards = myCards1.query.all()  # Obtiene todos los registros de la tabla de productos
    result = myCards1_schema.dump(all_cards)  # Serializa los registros en formato JSON
...
'''
@app.route("/myCards1/<id>", methods=["GET"])
def get_myCard(id):
    """
    ...
    Retorna un JSON con la información del producto correspondiente al ID proporcionado.
    """
    card = myCards1.query.get(id)  # Obtiene el producto correspondiente al ID recibido
    return myCard_schema.jsonify(card)  # Retorna el JSON del producto

@app.route("/myCards1/<id>", methods=["DELETE"])
def delete_card(id):
    """
    ...
    """
    card = myCards1.query.get(id)  # Obtiene el producto correspondiente al ID recibido
    db.session.delete(card)  # Elimina el producto de la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return myCard_schema.jsonify(card)  # Retorna el JSON del producto eliminado

@app.route("/myCards1", methods=["POST"])  # Endpoint para crear un producto
def create_card():
    """
    ...
    Retorna un JSON con el nuevo producto creado.
    """
    city = request.json["city"]  # Obtiene el nombre del producto del JSON proporcionado
    country = request.json["country"]  # Obtiene el precio del producto del JSON proporcionado
    urlImg = request.json["urlImg"]  # Obtiene el stock del producto del JSON proporcionado
    description = request.json["description"]  # Obtiene la imagen del producto del JSON proporcionado
    new_card = myCards1(city, country, urlImg, description)  # Crea un nuevo objeto Producto con los datos proporcionados
    db.session.add(new_card)  # Agrega el nuevo producto a la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return myCard_schema.jsonify(new_card)  # Retorna el JSON del nuevo producto creado

@app.route("/myCards1/<id>", methods=["PUT"])  # Endpoint para actualizar un producto
def update_card(id):
    """
